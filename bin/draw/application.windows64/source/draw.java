...
float arrowangle=radians(20.0f);
...
//vertices
    ...
    for(int i=0;i<num;i++){
      if(adj[i][i]){
        noFill();
        ellipse(cx+x[i]/scale-radius,cy+y[i]/scale-radius,2.8284f*radius,2.8284f*radius); //2.8284 = 2*sqrt(2)
      }
...
public void importgraph(){     
   ...
   if(lines.length>=2*num+3){     
     ...
     for(int i=0; i<parts.length; i++){
       ...
       vertexColoring.put(PApplet.parseInt(subparts[0]), PApplet.parseInt(subparts[1]));
     }
   }
   ...
   if(lines.length>=2*num+4){     
     ...
     for(int i=0; i<parts.length; i++){
       ...
       edgeColoring.put(subparts[0], PApplet.parseInt(subparts[1]));
     }
   }   
}
...
public void exportgraph(){  
    ...
    //coordinates
     for(int i=0;i<num;i++){
       parts[0]=str(PApplet.parseInt(cx+x[i]/scale-centerX));
       parts[1]=str(PApplet.parseInt(cy+y[i]/scale-centerY));
